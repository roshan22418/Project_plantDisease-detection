...
class PlantDiseaseModel(nn.Module):
    def _init_(self, num_classes=45):
...
def get_advice(disease_name, llm_option="gpt4o_mini"):
    ...
    formatted_disease = disease_name.replace('', ' - ').replace('', ' ')
...
def main():
    ...
    if LLM_CONFIG[st.session_state.llm_option].get("needs_api_key", False):
        ...
        if api_key:
            ...
            st.sidebar.warning("⚠ API key required for this model")
                    ...
                    with st.spinner('Analyzing image...'):
                        ...
                        with col2:
                            ...
                            if confidence < threshold:
                                ...
                                st.markdown(f"### Detected Disease: *{predicted_class.replace('', ' - ').replace('', ' ')}*")
                                ...
                                for i in range(1, 3):
                                    ...
                                    st.markdown(f"- {alt_class.replace('', ' - ').replace('', ' ')}: {alt_confidence:.2f}%")
